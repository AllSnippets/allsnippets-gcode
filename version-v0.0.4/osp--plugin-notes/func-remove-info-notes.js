(function () {
    ...
    function gatherDeleteInfoNotesFormData() {
        const formData = new FormData();
        
        formData.append('action', `${slugUnderscored}__ajax__delete_all_info_notes`);
    ...
    // --- 5. AJAX FETCH --- //
    function deleteInfoNotes() {
        const formData = gatherDeleteInfoNotesFormData();
    ...
    function handleDeleteResponse(resp) {
        if (resp && resp.success) {
            ...
            console.error('[REMOVE INFO NOTES] Error:', resp);
            hideLoadingIndicator();
            alert('Error deleting info_notes values: ' + (resp.data && resp.data.message ? resp.data.message : 'Unknown error'));
        }
    }
    ...
    function handleAjaxError(error) {
        console.error('[REMOVE INFO NOTES] AJAX error:', error);
        hideLoadingIndicator();
