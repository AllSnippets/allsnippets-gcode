(function () {
    ...
    const sidebarRootSelector = '.gp-sidebar--root';
    ...
    // --- 3. EVENT LISTENERS --- //
    // Delegiran listener za toggle gumb
    ...
    function handleSidebarToggle(toggleBtn) {
        const root = toggleBtn.closest(sidebarRootSelector);
        if (!root) return;
        ...
        // Preklopi vidnost sidebara
        root.classList.toggle(hiddenClass);

        // Dodaj/odstrani class na vseh elementih z adaptive margin
        adaptiveMargins.forEach(function (adaptiveMargin) {
            adaptiveMargin.classList.toggle(sidebarHiddenClass);
        });

        // Posodobi ikono puščice
        updateToggleIcon(root, icon);
    }

    function updateToggleIcon(root, icon) {
        if (root.classList.contains(hiddenClass)) {
