(function () {
    ...
    document.addEventListener('change', function (e) {
        ...
        if (e.target.matches(columnCheckboxSelector)) {
            ...
            if (column) {
                ...
                // Za folder stolpce smo potrebovali narediti še poseben handler
                const folderElements = document.querySelectorAll('.gp-sfold--main-table--td-cell--root [data-column="' + column + '"]');

                if (checkbox.checked) {
                    columnElements.forEach(el => el.style.removeProperty('display'));
                    folderElements.forEach(el => el.style.removeProperty('display'));
                } else {
                    columnElements.forEach(el => el.style.setProperty('display', 'none', 'important'));
                    folderElements.forEach(el => el.style.setProperty('display', 'none', 'important'));
                }
            }
    ...
    function gatherUserPrefFormData(orderby, order) {
        ...
        formData.append('action', 'all_snippets__ajax__save_userpref_admin_page__vsh0_0_3');
